...
// Ronnys eigene Wortherkunfts-Anmerkung aus der Reyknap-Liste (z.B.
// "hay (naechstes) + alo (Mal) + vay (bis)") - kuratierter Eigeninhalt,
// kein Woerterbuch-Dump, daher vertraeglich mit der "keine Vokabeldaten
// lokal speichern"-Regel. Wird woechentlich von
// scripts/sync-priority-from-sheet.js aus dem Sheet aktualisiert und im
// Trainer auf der Na'vi-Kartenseite angezeigt. Nur Deutsch (Quellsprache
// des Sheets), das UI-Label ist uebersetzt.
const vocabPriorityColumns = db.prepare("PRAGMA table_info(vocab_priority)").all();
const hasOriginNoteColumn = vocabPriorityColumns.some((col) => col.name === 'origin_note');
if (!hasOriginNoteColumn) {
  db.exec('ALTER TABLE vocab_priority ADD COLUMN origin_note TEXT');
}
...
// Zeitpunkt des letzten erfolgreichen Logins (siehe routes/auth.js) - bisher
// gab es keine Moeglichkeit nachzuvollziehen, ob/wann ein Konto (insb. die
// oeffentlichen Demo-Konten) tatsaechlich genutzt wurde.
const usersColumns4 = db.prepare('PRAGMA table_info(users)').all();
if (!usersColumns4.some((col) => col.name === 'last_login')) {
